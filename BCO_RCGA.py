...
# =============================
# FITNESS FUNCTION
# Encoding: Real-valued [0, 1] per validator
...
# Standard crossover operator for Real-Coded GA
# Produces offspring that respect the bounds of parents
# =============================
...
# =============================
# POLYNOMIAL MUTATION
# Standard mutation operator for Real-Coded GA
# Perturbs gene values within [0, 1] using polynomial distribution
# =============================
def polynomial_mutation(individual, eta_m=20.0, mut_prob=0.08):
...
# Selects the better individual from a random pair
# =============================
...
# =============================
# RCGA SINGLE RUN
# Encoding: Real-valued chromosome of length NUM_VALIDATORS
# Each gene ∈ [0, 1]; gene > 0.5 means validator is selected
# =============================
def run_rcga_once(stake, latency, reputation, print_gen=False):
    ...
    for gen in range(1, MAX_GEN + 1):
        ...
        # Generate new population using SBX crossover + polynomial mutation
        new_pop = []
        while len(new_pop) < POP_SIZE:
            ...
            child1 = polynomial_mutation(child1, eta_m=20.0, mut_prob=0.08)
            child2 = polynomial_mutation(child2, eta_m=20.0, mut_prob=0.08)
...
# MULTI RUN
# =============================
def run_rcga_multiple(stake, latency, reputation, name):
    utilities = []
    all_histories = []
    ...
    for r in range(RUNS):
        if r == 0:
            U, m, n, idx, history = run_rcga_once(stake, latency, reputation, print_gen=True)
            ...
            U, m, n, idx, history = run_rcga_once(stake, latency, reputation)
    ...
    plt.figure()
    for i in range(RUNS):
        plt.plot(all_histories[i], alpha=0.3, color='steelblue')
    plt.plot(avg_curve, linewidth=2, color='navy', label='Average')
    plt.xlabel("Generation")
    plt.ylabel("Best Fitness")
    plt.title(f"RCGA Convergence ({name})")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    run_rcga_multiple(stake, latency, reputation, "Setting 1")
    run_rcga_multiple(stake_2, latency_2, reputation_2, "Setting 2")