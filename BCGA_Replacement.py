...
def is_feasible_individual(individual, m, n, resource, capacity):
    # Quick bool check for an individual
    used = np.zeros(m)
    for j in range(n):
...
# ---------------------------------------------------------------
# FEASIBLE INDIVIDUAL CONSTRUCTION
# ---------------------------------------------------------------
...
def fitness_raw(individual, m, n, cost):
    # Pure profit — valid since replacement keeps population feasible
...
# ---------------------------------------------------------------
# RUN BCGA — REPLACEMENT METHOD
# ---------------------------------------------------------------
def run_bcga_replacement(m, n, cost, resource, capacity):
    ...
    for gen in range(GENERATIONS):
        ...
        while len(new_pop) < POP_SIZE:
            ...
            # REPLACEMENT: infeasible offspring replaced by new feasible individual
            if not is_feasible_individual(c1, m, n, resource, capacity):
                c1 = create_feasible_individual(m, n, cost, resource, capacity)
            if not is_feasible_individual(c2, m, n, resource, capacity):
                c2 = create_feasible_individual(m, n, cost, resource, capacity)
...
if __name__ == "__main__":
    ...
    print(SEP)
    print("  Generalized Assignment Problem — BCGA [REPLACEMENT]")
    print(SEP)
    ...
    print(f"  Constraint method  : replacement (infeasible offspring replaced by feasible individual)")
    print(LINE)
    ...
    for run in range(NUM_RUNS):
        ...
        t0 = time.time()
        sol, val, history = run_bcga_replacement(m, n, cost, resource, capacity)
        rt = time.time() - t0
