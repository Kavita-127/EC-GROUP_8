...
NUM_GENERATIONS  = 200
NUM_VARIABLES    = 10
MIN_BOUND = np.array([0.0]  + [-5.0] * (NUM_VARIABLES - 1))
MAX_BOUND = np.array([1.0]  + [ 5.0] * (NUM_VARIABLES - 1))
...
# ==========================================================
# PROBLEM FUNCTIONS (ZDT4)
# ==========================================================
def evaluate_objectives(chromosome):
    """Return (f1, f2) for ZDT4."""
    chromosome  = np.asarray(chromosome)
    objective_1 = chromosome[0]
    aux_g       = 1 + 10 * (len(chromosome) - 1) + np.sum(chromosome[1:] ** 2 - 10 * np.cos(4 * np.pi * chromosome[1:]))
    aux_h       = 1 - np.sqrt(objective_1 / aux_g)
...
def polynomial_mutation(individual):
    ...
    for gene_idx in range(len(mutated)):
        ...
        mutated[gene_idx] += perturbation * (MAX_BOUND[gene_idx] - MIN_BOUND[gene_idx])
...
def plot_results(initial_obj, pareto_obj):
    true_front_f1 = np.linspace(0, 1, 200)
    true_front_f2 = 1 - np.sqrt(true_front_f1)
    ...
    plt.plot(true_front_f1, true_front_f2, 'r-', linewidth=2, label='True Pareto front')
