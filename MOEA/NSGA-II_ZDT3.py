...
def plot_results(initial_F, final_F):
    ...
    # True Pareto front (ZDT3) — 5 discontinuous segments
    regions = [
        (0.000, 0.0830),
        (0.182, 0.2577),
        (0.409, 0.4538),
    ...
    for idx, (f1_seg, f2_seg) in enumerate(zip(f1_segs, f2_segs)):
        plt.plot(f1_seg, f2_seg, '--',
                 color="#DD8452",
                 linewidth=2,
                 label="True Pareto Front" if idx == 0 else "")

    plt.title("Initial Population", fontsize=12, fontweight='bold')
    plt.xlabel("f1")
    plt.ylabel("f2")
    plt.legend()
    ...
    plt.title("Final Pareto Front", fontsize=12, fontweight='bold')
    plt.xlabel("f1")
    plt.ylabel("f2")
    plt.legend()

    plt.suptitle("NSGA-II on ZDT3", fontsize=14)
    plt.tight_layout()
    plt.show()
