...
class GAP_RCGA:
    ...
    def is_feas(self, ind):
        return is_feasible_bool(self.decode(ind), self.m, self.n,
                                self.resource, self.capacity)

    def fit_sel(self, ind):
        return fitness_raw(self.decode(ind), self.m, self.n, self.cost)
    ...
    def tournament(self, pop):
        ...
        return max(sel, key=self.fit_sel)
    ...
    def run(self):
        ...
        for _ in range(ITERATIONS):
            ...
            while len(new) < POP_SIZE:
                ...
                c1 = self.pmut(c1); c2 = self.pmut(c2)
                if not self.is_feas(c1):
                    c1 = self.create_individual()
                if not self.is_feas(c2):
...
if __name__ == "__main__":
    ...
    print_header("RCGA", "replacement", filepath, problems, m, n,
    ...
    plot_results(np.mean(all_hist,axis=0), rtimes,
                 "RCGA","replacement", filepath, "#D85A30")