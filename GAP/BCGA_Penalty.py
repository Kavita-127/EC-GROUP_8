...
SEED           = 42
POP_SIZE       = 100       # >= 100
GENERATIONS    = 800       # 700-800
CROSSOVER_RATE = 0.8       # 0.7 or 0.8
MUTATION_RATE  = 0.15      # 0.1 - 0.2
NUM_RUNS       = 20
...
# ---------------------------------------------------------------
# INDIVIDUAL ENCODING  (integer assignment list)
# assignment[j] = agent index for job j
# ---------------------------------------------------------------
def create_feasible_individual(m, n, cost, resource, capacity):
    """
    Greedy construction: assigns each job to the highest-profit
    feasible agent. Shuffled job order ensures population diversity.
    All individuals are feasible from the start.
    """
    used      = np.zeros(m)
    assign    = [0] * n
...
def repair_individual(individual, m, n, cost, resource, capacity):
    # Repair capacity violations greedily
    used = np.zeros(m)
    for j in range(n):
    ...
    for _ in range(2000):
        ...
        jobs.sort(key=lambda j: cost[i][j])
        moved = False
        for j in jobs:
            for k in range(m):
...
def compute_penalty(m, n, cost):
    max_profit = sum(int(cost[:, j].max()) for j in range(n))
    return max_profit + 1


def fitness_penalty(individual, m, n, cost, resource, capacity, penalty):
    # Guided fitness
...
def mutate(individual, m, n, cost, resource, capacity):
    ind = individual[:]
    for j in range(n):
        if random.random() < MUTATION_RATE:
            ind[j] = random.randint(0, m - 1)
    # Repair after mutation to keep feasibility pressure
    return repair_individual(ind, m, n, cost, resource, capacity)


# ---------------------------------------------------------------
# RUN BCGA — PENALTY METHOD
# ---------------------------------------------------------------
def run_bcga_penalty(m, n, cost, resource, capacity):

    PENALTY = compute_penalty(m, n, cost)  # auto-scaled, > max profit

    sel_fit = lambda x: fitness_penalty(x, m, n, cost, resource, capacity, PENALTY)

    # Requirement 7: all initial individuals are feasible
    population = [create_feasible_individual(m, n, cost, resource, capacity)
                  for _ in range(POP_SIZE)]

    # Seed best from gen-0 using true fitness
    ...
    for gen in range(GENERATIONS):
        # Elitism: carry best feasible individual
        new_pop = [best_sol[:]]
        ...
        while len(new_pop) < POP_SIZE:
            ...
            c1 = mutate(c1, m, n, cost, resource, capacity)
            c2 = mutate(c2, m, n, cost, resource, capacity)
            new_pop.append(c1)
            if len(new_pop) < POP_SIZE: new_pop.append(c2)

        population = new_pop[:POP_SIZE]

        # Requirement 8: update best only from feasible solutions (penalty=0)
...
if __name__ == "__main__":
    ...
    PENALTY = compute_penalty(m, n, cost)

    print(SEP)
    print("  Generalized Assignment Problem — BCGA [PENALTY]")
    print(SEP)
    ...
    print(f"  Penalty per unit   : {PENALTY}  (> max profit {PENALTY-1}, guarantees feasible best)")
    print(f"  Constraint method  : penalty")
    print(LINE)
    ...
    for run in range(NUM_RUNS):
        ...
        t0 = time.time()
        sol, val, history = run_bcga_penalty(m, n, cost, resource, capacity)
        rt = time.time() - t0
    ...
    axes[1].set_title(f"BCGA [PENALTY] Runtime per Run\n({GENERATIONS} generations, pop={POP_SIZE})")
